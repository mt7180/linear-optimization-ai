...
class PyomoParam(BaseModel):
    ...
    initialize: dict[Union[int, tuple[int,...]], Any] = Field(..., description="data dict for parameter initialization with `index` values as keys.") # or rule: rule is currently not possible
    within: Optional[str]
    doc: str = Field(..., description="short description")
    ...
    def convert_key(key: str) -> int | tuple[int]:
        ...
        try:
            return tuple(int(k) for k in key.strip("()").split(',')) #[1:-1].split(','))
        except ValueError:
...
class LinearOptimizationModel(BaseModel):
    mathematical_formulation: str = Field(...,description="latex representation of only the mathematical formulation of the given optimization problem")
