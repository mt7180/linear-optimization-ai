    ...
    def _check_node(node):
        ...
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Name):
                return node.func.id in allowed_math_functions and all(
                    _check_node(arg) for arg in node.args
                )
            return (
                isinstance(node.func, ast.Attribute)
                and _check_node(node.func)
                and all(_check_node(arg) for arg in node.args)
            )

        if isinstance(node, ast.Name):
            logging.debug("checking variable: ")
            logging.debug(f"{node.id}")
            return node.id in allowed_vars or node.id in iteration_vars

        if isinstance(node, ast.Attribute):
            logging.debug("checking attributes: ")
        ...
        if isinstance(node, ast.Compare):
            ...
            return (
                ...
                and all(
                    isinstance(op, (ast.Eq, ast.Gt, ast.Lt, ast.GtE, ast.LtE))
                    for op in node.ops
                )
                and all(_check_node(comp) for comp in node.comparators)
            )
        ...
        if isinstance(node, ast.ListComp) or isinstance(node, ast.GeneratorExp):
            ...
            return _check_node(node.elt) and all(
                _check_node(gen.iter) for gen in node.generators
            )
        ...
        if isinstance(node, ast.IfExp):
            ...
            return (
                ...
                and _check_node(node.body)
                and _check_node(node.orelse)
            )
...
def parse_rule(expression: str, allowed_vars: Collection[str]) -> dict[str, Any]:
    ...
    if "lambda" in expression:
        ...
        lambda_args = [
            arg.strip()
            for arg in expr_left[expr_left.find("lambda") + 6 :].strip().split(",")
        ]
        expression = expr_right.strip()
    else:
        lambda_args = ["model"]

    expression_is_safe = check_if_expression_is_safe(
        expression, set(allowed_vars) | set(lambda_args)
    )
...
if __name__ == "__main__":
    ...
    expression = (
        "sum(m.x[(d+ working_day)%7]  for working_day in m.wd)  >= m.epsilon[d]"
    )
