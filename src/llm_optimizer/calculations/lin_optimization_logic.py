...
def construct_pyomo_model(llm_pyomo_model: LinearOptimizationModel) -> pyo.ConcreteModel:
    ...
    for pyo_constr in llm_pyomo_model.constraints:
        allowed_vars=(
            {"model." + attr.name for attr in model.component_objects()} 
            | {*{comp_key for comp in model.component_objects() for comp_key in comp.keys() if comp_key}}
        )

        logging.debug(f"creating constraint {pyo_constr.rule} with allowed vars: {allowed_vars}")
        ...
        if getattr(pyo_constr, "expr", None):
            ...
            rule_str = "lambda " + ", ".join(pyo_constr.rule.lambda_arguments) +": " + pyo_constr.rule.lambda_body
            logging.debug("check: " + rule_str +", " + str(pyo_constr.idxs))
            rule = parse_rule(rule_str, allowed_vars | set(pyo_constr.rule.lambda_arguments))
            create_constraint(pyo_constr.idxs, model, pyo_constr.name, rule=rule, doc=pyo_constr.doc)
        else:
            raise RuleError("Constraint must have either rule or expression.") 
    
    logging.debug(f"creating objective {llm_pyomo_model.objective.expr or llm_pyomo_model.objective.rule}")
